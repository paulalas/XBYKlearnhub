...
Page templates should be organized in the `~/PageTemplates` folder:

```
PageTemplates/
├── _ViewImports.cshtml          (Kentico namespaces and tag helpers)
├── _ViewStart.cshtml             (Optional: sets default layout)
└── [TemplateName]/
...
**Minimal Template Example (RECOMMENDED):**
```cshtml
...
- Include FULL HTML page (`<html>`, `<head>`, `<body>`)
- Include `<page-builder-styles />` in the `<head>`
- Include `<page-builder-scripts />` before closing `</body>`
- Prefix the filename with underscore: `_TemplateName.cshtml`
- Use `<editable-area area-identifier="main" />` for Page Builder content
- Keep templates minimal - NO header/footer (those go in _Layout.cshtml)

**❌ DON'T:**
- Don't forget the underscore prefix on the filename
- Don't include site-wide header/footer in the template
- Don't use `<page-builder-editable-area />` - use `<editable-area />` tag helper
- Don't forget `<page-builder-styles />` and `<page-builder-scripts />`
...
### 6. Create _ViewImports.cshtml (if not exists)

**Location:** `~/PageTemplates/_ViewImports.cshtml`
...
### 7. Register Content Type for Page Builder
...
[assembly: RegisterPageTemplate(
    identifier: "BoilerPage.Home_Default",
    name: "Home - Default",
    propertiesType: typeof(PageTemplates.Home.HomePageTemplateProperties),
    customViewName: "~/PageTemplates/Home/_Home.cshtml",
    ContentTypeNames = new[] { BoilerPage.Home.CONTENT_TYPE_NAME },
    Description = "Default template for Home page",
    IconClass = "xp-layout")]
...
- [ ] Editable areas work in admin (using `<editable-area />` syntax)
- [ ] `<page-builder-styles />` and `<page-builder-scripts />` are included
- [ ] Build succeeds without errors
- [ ] No namespace conflicts
- [ ] Model type matches registration
